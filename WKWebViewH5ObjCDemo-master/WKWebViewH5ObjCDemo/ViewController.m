...
- (void)viewDidLoad {
  ...
  [config.userContentController addScriptMessageHandler:self name:@"jsInterface"];
  
  self.webView = [[WKWebView alloc] initWithFrame:self.view.bounds
                                    configuration:config];
//  NSURL *path = [[NSBundle mainBundle] URLForResource:@"test" withExtension:@"html"];
    NSURL *URL = [NSURL URLWithString:@"http://192.168.11.112:8080/phapp/f/ph/cdc/toolIndex?crmId=15954008491"];
...
- (void)userContentController:(WKUserContentController *)userContentController
  ...
  if ([message.name isEqualToString:@"jsInterface"]) {
    // 打印所传过来的参数，只支持NSNumber, NSString, NSDate, NSArray,
    // NSDictionary, and NSNull类型
    NSLog(@"参量：%@", message.body);
      NSLog(@"name:%@",message.name);
      NSLog(@"frame:%@",message.frameInfo);
      NSLog(@"web:%@",message.webView);
//      message.pos
  }
}
